...
#define GST_G_IO_MODULE_DECLARE(name) extern void G_PASTE(g_io_, G_PASTE(name, _load))(gpointer data)
...
static void gst_android_load_gio_modules(void) {
    ...
    if (backend && ca_certs) {
        ...
        if (db) {
            ...
            g_warning("Failed to create a database from file: %s", error ? error->message : "Unknown");
        }
    }
}
